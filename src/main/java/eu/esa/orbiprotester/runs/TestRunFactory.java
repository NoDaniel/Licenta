...
public final class TestRunFactory {
    ...
    public static TestRun buildTestRun(final File input, final File baseOutputFolder, final File referenceFolder, final Logger logger) throws IOException {
        ...
        switch (type) {
        case EVENT:
        	testRun = new EventTestRun(parser, input, outputFolder, referenceFolder, testName, logger);
        	break;
        case FORMATION:
            testRun = new FormationFlightTestRun(parser, input, outputFolder, referenceFolder, testName, logger);
            break;
        case ORBITCONVERSION:
            testRun = new OrbitConversionTestRun(parser, input, outputFolder, referenceFolder, testName, logger);
            break;
        case DATASERIESCOMPARE:
            testRun = new DataSeriesCompareTestRun(parser, input, outputFolder, referenceFolder, testName, logger);
            break;
        case XMLVALIDATION:
            testRun = new XmlValidationTestRun(parser, input, outputFolder, referenceFolder, testName, logger);
            break;
        case STANDARD:
        default:
