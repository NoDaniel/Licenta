...
public abstract class AbstractTestRun implements TestRun {
    ...
    /** Get the configured eclipse detector
     * @param handler the handler for the event. If null no handler is used
     * @return the event detector
     * @throws OrekitException
     */
    protected EventDetector getEventDetector(EventDetectionHandler handler) throws OrekitException {
        AbstractDetector<? extends EventDetector> ad = null;
        
        // read the event. Stop on first match.
        if(parser.containsKey(ParameterKey.ORBIPRO_EVENTS_APSIDE) && 
                parser.getBoolean(ParameterKey.ORBIPRO_EVENTS_APSIDE)) {
            
            if (handler != null) {
                if (parser.containsKey(ParameterKey.ORBIPRO_EVENTS_APSIDE_PERIGEE)) {
                    if(parser.getBoolean(ParameterKey.ORBIPRO_EVENTS_APSIDE_PERIGEE)) {
                        // Perigee detector
                        handler.setDirection(Direction.INCREASING);
                    } else {
                        //Apogee detector
                        handler.setDirection(Direction.DECREASING);
                    }
                } else {
                    // Both apogee and perigee crossing are counted.
                    handler.setDirection(Direction.ALL);
                }
            }
                                
            // apside event
            ad = new ApsideDetector(getOrbit());
        }

        // read the event. Stop on first match.
        if(parser.containsKey(ParameterKey.ORBIPRO_EVENTS_NODE) && 
           parser.getBoolean(ParameterKey.ORBIPRO_EVENTS_NODE)) {
            
            if (handler != null) {
                if (parser.containsKey(ParameterKey.ORBIPRO_EVENTS_NODE_ASCENDING)) {
                    if(parser.getBoolean(ParameterKey.ORBIPRO_EVENTS_NODE_ASCENDING)) {
                        //Ascending node crossing detector
                        handler.setDirection(Direction.INCREASING);
                    } else {
                        //Descending node crossing detector
                        handler.setDirection(Direction.DECREASING);
                    }
                } else {
                    // Both ascending and descending node crossing are counted.
                    handler.setDirection(Direction.ALL);
                }
            }
                                
            // node detector event
            ad = new NodeDetector(getOrbit(), getFrame());
        }
        
        // read the event. Stop on first match.
        if(parser.containsKey(ParameterKey.ORBIPRO_EVENTS_ELEVATION) && 
           parser.getBoolean(ParameterKey.ORBIPRO_EVENTS_ELEVATION)) {
            
            if (handler != null) {
                if (parser.containsKey(ParameterKey.ORBIPRO_EVENTS_ELEVATION_RAISING)) {
                    if(parser.getBoolean(ParameterKey.ORBIPRO_EVENTS_ELEVATION_RAISING)) {
                        //Ascending node crossing detector
                        handler.setDirection(Direction.INCREASING);
                    } else {
                        //Descending node crossing detector
            ...
            // get the station frame
            final double longitude = parser.getAngle(ParameterKey.ORBIPRO_EVENTS_ELEVATION_LONGITUDE);
            final double latitude = parser.getAngle(ParameterKey.ORBIPRO_EVENTS_ELEVATION_LATITUDE);
            final double altitude = parser.getDouble(ParameterKey.ORBIPRO_EVENTS_ELEVATION_ALTITUDE);
            
            final GeodeticPoint station = new GeodeticPoint(latitude, longitude, altitude * 1000.);
            final TopocentricFrame stationFrame = new TopocentricFrame(getCentralBodyShape(), station, "StationFrame");
            
            // get the elevation, if set
            double minElevation = 0;
            if (parser.containsKey(ParameterKey.ORBIPRO_EVENTS_ELEVATION_MIN)) {
                minElevation = parser.getAngle(ParameterKey.ORBIPRO_EVENTS_ELEVATION_MIN);
            }

            // check if refraction is required
            boolean refraction = false;
            if (parser.containsKey(ParameterKey.ORBIPRO_EVENTS_ELEVATION_REFRACTION)) {
                refraction = parser.getBoolean(ParameterKey.ORBIPRO_EVENTS_ELEVATION_REFRACTION);
            }
            
            
            // elevation detector event
            ElevationDetector eld = new ElevationDetector(stationFrame).withConstantElevation(minElevation);
                        
            ad = refraction ? eld.withRefraction(new EarthStandardAtmosphereRefraction()) : eld ; 
        }     

        // read the event. Stop on first match.
        if(parser.containsKey(ParameterKey.ORBIPRO_EVENTS_DATE) && 
           parser.getBoolean(ParameterKey.ORBIPRO_EVENTS_DATE)) {
            ...
            final AbsoluteDate startDate = parser.getDate(ParameterKey.ORBIPRO_EVENTS_DATE_START, getTs());
            final AbsoluteDate endDate = parser.getDate(ParameterKey.ORBIPRO_EVENTS_DATE_END, getTs());
            final double step = parser.getDouble(ParameterKey.ORBIPRO_EVENTS_DATE_STEP);
            
            // date detector event
            DateDetector dd = new DateDetector(step-1, 1e-9, startDate);
            AbsoluteDate newDate = startDate.shiftedBy(step);
            while (newDate.compareTo(endDate) <= 0) {
                dd.addEventDate(newDate);
                newDate = newDate.shiftedBy(step);
            }
        ...
        // read the event. Stop on first match.
        if(parser.containsKey(ParameterKey.ORBIPRO_EVENTS_ALTITUDE) && 
           parser.getBoolean(ParameterKey.ORBIPRO_EVENTS_ALTITUDE)) {
            ...
            // get the time interval
            final double altitude = parser.getDouble(ParameterKey.ORBIPRO_EVENTS_ALTITUDE_VALUE);
            
            // altitude detector event
            ad = new AltitudeDetector(50, altitude*1000., getCentralBodyShape()); 
        } 
        
        // read the event. Stop on first match.
        if(parser.containsKey(ParameterKey.ORBIPRO_EVENTS_ECLIPSE) && 
           parser.getBoolean(ParameterKey.ORBIPRO_EVENTS_ECLIPSE)) {
            
            if (handler != null) {
                if (parser.containsKey(ParameterKey.ORBIPRO_EVENTS_ECLIPSE_ENTER)) {
                    if(parser.getBoolean(ParameterKey.ORBIPRO_EVENTS_ECLIPSE_ENTER)) {
                        // Entering eclipse detector
                        handler.setDirection(Direction.DECREASING);
                    } else {
                        // Exiting eclipse detector
                        handler.setDirection(Direction.INCREASING);
                    }
                } else {
                    // Both enter and exit eclipse detector.
                    handler.setDirection(Direction.ALL);
                }
            }
            
            // Get the occulted body
            final CelestialBody occulted = this.parser.getCelectialBodies(ParameterKey.ORBIPRO_EVENTS_ECLIPSE_OCCULTED).get(0);
            final double occultedRadius = this.parser.getDouble(ParameterKey.ORBIPRO_EVENTS_ECLIPSE_OCCULTED_RADIUS); 
            
            // Get the occulting body
            final CelestialBody occulting = this.parser.getCelectialBodies(ParameterKey.ORBIPRO_EVENTS_ECLIPSE_OCCULTING).get(0);
            final double occultingRadius = this.parser.getDouble(ParameterKey.ORBIPRO_EVENTS_ECLIPSE_OCCULTING_RADIUS); 
            
            // eclipse detector event
            final EclipseDetector eclDet = new EclipseDetector(occulted, occultedRadius, occulting, occultingRadius);

            // Get the umbra/penumbra eclipse detector flag
            final boolean totalEclipse = parser.getBoolean(ParameterKey.ORBIPRO_EVENTS_ECLIPSE_TOTAL);
            if (totalEclipse) {
            	ad = eclDet.withUmbra();
            } else {
            	ad = eclDet.withPenumbra();
            }
            
        } 

        // read the event. Stop on first match.
        if(parser.containsKey(ParameterKey.ORBIPRO_EVENTS_ALIGNMENT) && 
           parser.getBoolean(ParameterKey.ORBIPRO_EVENTS_ALIGNMENT)) {
            ...
            // Get the body
            final CelestialBody body = this.parser.getCelectialBodies(ParameterKey.ORBIPRO_EVENTS_ALIGNMENT_BODY).get(0);
            final double angle = this.parser.getDouble(ParameterKey.ORBIPRO_EVENTS_ALIGNMENT_ANGLE); 
            
            ad = new AlignmentDetector(this.orbit, body, angle);
        } 
        ...
        if (handler != null) {
            ed = ad.withHandler(handler);
        } else {
            ed = ad.withHandler(new StopOnEvent<>());
        }

        // Check if event shifting is required
        if(parser.containsKey(ParameterKey.ORBIPRO_EVENTS_SHIFT) && 
           parser.getBoolean(ParameterKey.ORBIPRO_EVENTS_SHIFT)) {
            final double incOff = parser.getDouble(ParameterKey.ORBIPRO_EVENTS_SHIFT_INCREASEOFFSET);
            final double decOff = parser.getDouble(ParameterKey.ORBIPRO_EVENTS_SHIFT_DECREASEOFFSET);
            
            ed = new EventShifter<EventDetector>(ed, true, incOff, decOff);
        }
